...
ggplot(df,aes(isActive)) + geom_bar(aes(y=..count../sum(..count..)))
```
...
df$AgeYears <- time_length(as.period(interval(ymd_hms(df$LoanOriginationDate),ymd_hms(df$ClosedDate))),'years')
summary(df$AgeYears)
ggplot(df,aes(AgeYears)) + geom_density()
...
ggplot(subset(df,AgeYears >= 0),aes(AgeYears)) + geom_density()
```
...
df$AgeDaysToOrigination <- time_length(as.period(interval(ymd_hms(df$ListingCreationDate),ymd_hms(df$LoanOriginationDate))),'days')
summary(df$AgeDaysToOrigination)
ggplot(df,aes(AgeDaysToOrigination)) + geom_density()
ggplot(df,aes(AgeDaysToOrigination)) + geom_density() + xlim(c(0,25))
...
df$ListingCategory..numeric. = factor(df$ListingCategory..numeric.,
                                      ...
                                      labels = c("Not Available","Debt Consolidation","Home Improvement","Business","Personal Loan","Student Use","Auto","Other","Baby&Adoption","Boat","Cosmetic Procedures","Engagement Ring","Green Loans","Household Expenses","Large Purchases","Medical/Dental","Motorcycle","RV","Taxes","Vacation","Wedding Loans"))
table(df$ListingCategory..numeric.)
prop.table(table(df$ListingCategory..numeric.))
ggplot(df,aes(ListingCategory..numeric.)) + geom_bar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
ggplot(df,aes(LoanOriginalAmount)) + geom_density()
ggplot(df,aes(LoanOriginalAmount)) + geom_bar() 
```
...
ggplot(df, aes(reorder(BorrowerState, -table(BorrowerState)[BorrowerState]))) + geom_bar()

df_state_count <- summarize(group_by(df,BorrowerState),
          count = length(BorrowerState))

df_state_count$BorrowerStateFull <- tolower(state.name[match(df_state_count$BorrowerState, state.abb)])
states <- map_data("state")
states$region <- tolower(states$region)
df_state_count <- merge(df_state_count,states,by.x = "BorrowerStateFull",by.y = "region")
...
ggplot(df, aes(EmploymentStatusDuration)) + 
  geom_bar(binwidth=6) + ggtitle("Binwidth = 6 months")
...
prop.table(table(df$CreditRange))*100
...
50% of borrowers have a utilization between .3 and .84. Median BankcardUtilization is around .6.  That means, at the time the credit profile was pulled, around 60% of available credit was being used by the median borrower. 7% (7604) of loans had a value of NA for BankcardUtilization/RevolvingCreditBalance. A possible explanation for this is that 7% of originated loans are for borrowers with no credit card at the time of applying for the loan. It's a very interesting distribution, as there are peaks around 0, and 1, and a long tail after 1 (for people who's utilization is more than their available credit). 

I wonder how much the typical prosper borrower earns?

```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE}
ggplot(df,aes(IncomeRange)) + geom_bar()
summary(df$IncomeRange)
prop.table(table(df$IncomeVerifiable))
```

Most borrowers (56%) in the $25k-$75k annual income. And, 92% of borrowers can prove it. If i'm an investor, its reassuring that the borrower can validate their income source! For those borrowers who's income is Not Displayed (7%)/Not Employed (<1%), I'm surprised they were approved for a loan and that they got funded. 
...
```
.07% of loans were not fully funded -- But, seems like every originated loan is at least 70% funded. So, maybe the platform covers the rest? Not sure how this works.
...
ggplot(df,aes(DebtToIncomeRatio)) + geom_bar() + xlim(0,2)
...
```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE}
summary(df$DebtToIncomeRatio)
ggplot(df,aes(log10(DebtToIncomeRatio+1))) + geom_bar()
...
ggplot(df,aes(LossRate)) + geom_bar()
ggplot(subset(df,ClosedDate != "" & LossRate>0),aes(LossRate)) + geom_bar()
...
df$ProsperRating..Alpha. <- factor(df$ProsperRating..Alpha.,levels(df$ProsperRating..Alpha.)[c(3,2,4:8,1)])
table(df$ProsperRating..Alpha.)
ggplot(df,aes(LoanStatus)) + geom_bar() + facet_wrap(~ProsperRating..Alpha.) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

```

Okay, the bottom right plot shows that there's a bunch of loans with no Proser Rating. So, prosper introduced this score July 2009. Therefore, 29084 loans have no "prosper rating". We may see the CreditGrade for the rating that these scores were given.

```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE}
#Reorder levels of CreditGrade
df$CreditGrade <- factor(df$CreditGrade,levels(df$CreditGrade)[c(3,2,4:9,1)])
prop.table(table(df$CreditGrade))
...
- Prosper used to use a CreditGrade that looks like it was entirely based on the Credit Score of the borrower. In the table above, there are no overlaps between credit intervals. Here's a plot that shows this:
...
df_prejuly2009_sum <- summarise(group_by(subset(df,CreditGrade != ""),CreditGrade),
          ...
          chargeoff_or_default = round(sum(LoanStatus %in% c("Chargedoff","Defaulted"))/total,2))

df_prejuly2009_sum
ggplot(df_prejuly2009_sum,aes(CreditGrade,completed)) + geom_bar(stat='identity') 
ggplot(df_prejuly2009_sum,aes(CreditGrade,chargeoff_or_default)) + geom_bar(stat='identity') 
...
```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE}
#Reorder levels of CreditGrade
ggplot(subset(df,ClosedDate != "" & ProsperRating..Alpha. != ""),aes(ProsperRating..Alpha.,fill=LoanStatus)) + 
  geom_bar() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

df_postjuly2009_sum <- summarise(group_by(subset(df,ClosedDate != "" & ProsperRating..Alpha. != ""),ProsperRating..Alpha.),
          total = length(LoanStatus),
          cancelled = round(sum(LoanStatus=="Cancelled")/total,2),
          completed = round(sum(LoanStatus=="Completed")/total,2),
          chargeoff_or_default = round(sum(LoanStatus %in% c("Chargedoff","Defaulted"))/total,2))
...
ggplot(df_postjuly2009_sum,aes(ProsperRating..Alpha.,completed)) + geom_bar(stat='identity') 

#ggplot() + 
##  geom_bar(df_postjuly2009_sum,aes(ProsperRating..Alpha.,chargeoff_or_default),stat='identity')  + 
# geom_bar(data = df_prejuly2009_sum,aes(CreditGrade,chargeoff_or_default),stat="identity") 
...
loss_across_categories <- summarise(group_by(subset(df,ClosedDate != ""),ListingCategory..numeric.),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
loss_by_credit_range <- summarise(group_by(subset(df,ClosedDate != ""),CreditRange),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
loss_by_state$BorrowerStateFull <- tolower(state.name[match(loss_by_state$BorrowerState, state.abb)])
states <- map_data("state")
states$region <- tolower(states$region)
df_state_count <- merge(loss_by_state,states,by.x = "BorrowerStateFull",by.y = "region")
ggplot(df_state_count,aes(long,lat)) + 
...
loss_by_is_homeowner <- summarise(group_by(subset(df,ClosedDate != ""),IsBorrowerHomeowner),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
df$IncomeRange <- factor(df$IncomeRange,levels(df$IncomeRange)[c(8,1,2,4,5,6,3,7)])
loss_by_incomeRange <- summarise(group_by(subset(df,ClosedDate != ""),IncomeRange),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
ggplot(subset(df,ClosedDate != ""),aes(DebtToIncomeRatio,LP_NetPrincipalLoss/LoanOriginalAmount)) + 
  geom_point() 

ggplot(subset(df,ClosedDate != ""),aes(log10(DebtToIncomeRatio+1),LP_NetPrincipalLoss/LoanOriginalAmount)) + 
  geom_point() 

fit <- lm(formula= LP_NetPrincipalLoss/LoanOriginalAmount ~ log10(DebtToIncomeRatio+1),data =subset(df,ClosedDate != "") )
...
loss_across_categories <- summarise(group_by(subset(df,ClosedDate != ""),Term, ListingCategory..numeric.),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
loss_across_employment_status <- summarise(group_by(subset(df,ClosedDate != ""),Term,EmploymentStatus),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE, fig.width=15,fig.height=15,}
loss_across_income_range <- summarise(group_by(subset(df,ClosedDate != ""),Term,IncomeRange),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE, fig.width=15,fig.height=15,}
loss_across_credit_range <- summarise(group_by(subset(df,ClosedDate != ""),Term,CreditRange),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE, fig.width=15,fig.height=15,}
loss_across_ci_range <- summarise(group_by(subset(df,ClosedDate != ""),IncomeRange,CreditRange),
          total = length(LoanStatus),
          total_net_loss = sum(LP_NetPrincipalLoss),
          total_originated = sum(LoanOriginalAmount),
          loss_per_loan = total_net_loss/total,
...
ggplot(loss_across_ci_range,aes(CreditRange,loss_rate)) + 
  geom_point(stat="identity",aes(size=log10(total_originated))) + 
  ggtitle("Loss Rate across Credit/Income Range")+ 
...
```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE}
fit <- lm(LossRate~BorrowerRate+Term+CreditRange+IncomeRange,subset(df,ClosedDate != ""))
summary(fit)
...
Will take a detour here and look how much Prosper has changed in various states over time in terms of originations and loss_rate

```{r, echo=FALSE,warning=FALSE,error=FALSE,message=FALSE, fig.width=15,fig.height=15,}
df$LoanOriginationDateYear <- year(df$LoanOriginationDate)
originated_by_state <- summarise(group_by(df,BorrowerState,LoanOriginationDateYear),
          total_originated = sum(LoanOriginalAmount))

originated_by_state$BorrowerStateFull <- tolower(state.name[match(originated_by_state$BorrowerState, state.abb)])
states <- map_data("state")
states$region <- tolower(states$region)
df_state_count <- merge(originated_by_state,states,by.x = "BorrowerStateFull",by.y = "region")
...
ggplot(subset(d,!Grade %in% c("","NC")),aes(Grade,loss_rate,fill=type,color=type)) + 
...
Change in Loss Rate by CreditGrade/ProsperRating Pre/Post July 2009. We can see that the Loss Rate improves in every grade after the loan grading system changes from the CreditGrade to the ProsperRating.
...
ggplot(subset(loss_across_income_range,IncomeRange !="Not displayed"),aes(IncomeRange,loss_rate,group=factor(Term),fill=factor(Term))) + 
  geom_bar(stat="identity",position="dodge") + 
...
Loss Rate falls for all groups. Variation in the loss rate across Income Ranges increases as Term size increases. Finally, for high-income borrowers (>$100,000) of 36/60 month loans, the loss rate is nearly the same. 

### Plot Three
```{r echo=FALSE, fig.width=15,fig.height=15, Plot_Three}
df$LoanOriginationDateYear <- year(df$LoanOriginationDate)
originated_by_state <- summarise(group_by(df,BorrowerState,LoanOriginationDateYear),
          total_originated = sum(LoanOriginalAmount),
...
originated_by_state$BorrowerStateFull <- tolower(state.name[match(originated_by_state$BorrowerState, state.abb)])
dc <- dcast(data = originated_by_state,formula = BorrowerStateFull ~ LoanOriginationDateYear,fun.aggregate = mean,value.var = "total_originated_log10")
dc[is.na(dc)] <- 0
dc_m <- melt(dc,id=c("BorrowerStateFull"))

states <- map_data("state")
states$region <- tolower(states$region)
df_state_count <- merge(dc_m,states,by.x = "BorrowerStateFull",by.y = "region",all.x=TRUE)
names(df_state_count)[2] <- 'LoanOriginationYear'
names(df_state_count)[3] <- 'LoanOriginationVolume_Log10'

ggplot(subset(df_state_count,LoanOriginationYear %in% c(2006,2007,2008,2009,2010,2011,2012,2013)),aes(long,lat)) + 
